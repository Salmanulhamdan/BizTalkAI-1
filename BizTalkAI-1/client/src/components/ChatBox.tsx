...
import { Phone, PhoneOff, Mic, MicOff, Volume2, VolumeX, X, Send } from "lucide-react";
...
export default function ChatBox({ ainager, isOpen, onClose }: ChatBoxProps) {
  ...
  useEffect(() => {
    if (state.transcription.length > 0) {
      const newMessages = state.transcription.map(transcript => ({
        id: transcript.id,
